...
bool test_jacobian_finite_difference() {
    std::cout << "Testing Jacobian finite difference verification..." << std::endl;
    ...
    Circuit::LimitContext limitCtx{x, x};
    ...
    for (int i = 0; i < N; ++i) {
        ...
        Circuit::LimitContext limitCtx_pert{x_pert, x};
        for (auto *nl : c.newtonElements) {
...
bool test_pnp_jacobian() {
    std::cout << "Testing PNP Jacobian finite difference verification..." << std::endl;
...
bool test_kcl_conservation() {
    ...
    auto &Vc = c.addElement<VoltageSource>(nC, Circuit::GND, 5.0);
    auto &Vb = c.addElement<VoltageSource>(nB, Circuit::GND, 0.8);
    auto &Ve = c.addElement<VoltageSource>(nE, Circuit::GND, 0.0);
    ...
    for (int node = 0; node < numNodes; ++node) {
        ...
        if (std::abs(x[node]) > 100.0) { // Unreasonable voltage
            std::cout << "Unreasonable voltage at node " << node << ": " << x[node] << std::endl;
            return false;
        }
    }
    ...
    if (Vc_val < 0.1 || Vb_val < 0.1 || Ve_val > 1.0) {
        std::cout << "BJT voltages out of expected range: "
                  << "Vc=" << Vc_val << ", Vb=" << Vb_val << ", Ve=" << Ve_val << std::endl;
        return false;
    }
...
int main() {
    ...
    if (!test_jacobian_finite_difference()) all_passed = false;
    if (!test_pnp_jacobian()) all_passed = false;
    if (!test_kcl_conservation()) all_passed = false;
