...
public:
  ...
  void computeLimitedVoltages(const LimitContext &ctx) override {
    ...
    double nVt = n_ * Vt_;
    double vcrit = nVt * std::log(nVt / (1.414 * Is_));
  ...
  DiodeShockleyNR(const std::string & /*name*/, NodeIndex anode,
      ...
      : a(anode), c(cathode), Is_(Is), n_(n), Vt_(Vt), gmin_(gmin), BV_(BV),
        IBV_(IBV) {
    if (Is_ <= 0.0)
      throw std::invalid_argument("DiodeShockleyNR: Is must be > 0");
    if (n_ <= 0.0)
      throw std::invalid_argument("DiodeShockleyNR: n must be > 0");
    if (Vt_ <= 0.0)
      throw std::invalid_argument("DiodeShockleyNR: Vt must be > 0");
    if (gmin_ < 0.0)
      throw std::invalid_argument("DiodeShockleyNR: gmin must be >= 0");
    if (BV_ < 0.0)
      throw std::invalid_argument("DiodeShockleyNR: BV must be >= 0");
    if (IBV_ <= 0.0)
      throw std::invalid_argument("DiodeShockleyNR: IBV must be > 0");
  }
