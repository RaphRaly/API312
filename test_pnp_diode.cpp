...
bool testPnpDiodeConnected() {
    ...
    c.addElement<BjtPnpEbersMoll>(n2, n1, n1, pnp_params);  // C, B, E all connected to n1 except C to n2
    ...
    double v_diode = x[n1];
    std::cout << "PNP diode voltage: " << v_diode << "V (expected ~0.7V)" << std::endl;
...
bool testDiodeForwardBias() {
    ...
    double v_diode = x[n1];
    std::cout << "Diode voltage: " << v_diode << "V (expected ~0.7V)" << std::endl;
...
bool testPnpCommonEmitter() {
    ...
    // Collector load
    c.addElement<VoltageSource>(nCollector, GND, -5.0);  // Negative voltage for PNP
...
int main() {
    ...
    if (!testPnpDiodeConnected()) all_passed = false;
    if (!testDiodeForwardBias()) all_passed = false;
    if (!testPnpCommonEmitter()) all_passed = false;

    if (all_passed) {
        std::cout << "\n🎉 All PNP/Diode tests PASSED! Ready for 2520 builder." << std::endl;
        return 0;
    } else {
