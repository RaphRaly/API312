...
HomotopyResult run2DHomotopy(bool verbose = true) {
  ...
  const double BLEEDER_G_START =
      1e-4;                         // ~10kΩ bleeder - stronger to prevent latch
  ...
  const vector<double> sourceSteps = {
      ...
      0.62, 0.64, 0.66, 0.68, 0.7,  0.72, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0};

  for (double s : sourceSteps) {
    // KEY INSIGHT: Engage feedback proportionally to sources to stabilize OUT!
    // At s=0, alpha=0.1 (R_fb=100kΩ). As sources ramp, feedback strengthens.
    // CRITICAL FIX: Never let feedback be truly open to avoid latch
    double alpha = max(0.1, s); // At s=1, alpha=1.0 (R_fb=10kΩ)

    Api2520Homotopy::updateSources(src, s, alpha, BLEEDER_G_START);

    bool ok = c.solveDc(x, 200, 1e-6, false, 30, nullptr,
                        true); // Disable internal source stepping
    ...
    if (!ok || diag.isLatched) {
      ...
      if (verbose)
        cout << "  -> Attempting pseudo-transient recovery..." << endl;
  ...
  while (alpha < ALPHA_TARGET && stepCount < maxSteps) {
    ...
    if (verbose) {
      double rfb = Api2520Homotopy::R_FB_NOMINAL /
                   max(alphaNext, Api2520Homotopy::ALPHA_MIN);
    ...
    if (!ok || !acceptable) {
      ...
      if (alphaStep <= ALPHA_STEP_MIN) {
        if (verbose)
          cout << "    -> Trying pseudo-transient at alpha=" << alphaNext
               << "..." << endl;
        x = xGood;
        Api2520Homotopy::updateSources(src, 1.0, alphaNext, bleederG);
        ok = c.solveDcPseudoTransient(x, 1e-3, 1e-6);
        ...
        if (ok && Api2520Homotopy::isAcceptable(diag)) {
          ...
          if (verbose)
            cout << "    -> Stuck at alpha=" << alphaGood << endl;
          break;
        }
      }
      continue;
  ...
  if (!finalOk || finalDiag.isLatched) {
    if (verbose)
      cout << "  -> Final solve latched, trying PT..." << endl;
    x = xGood;
  ...
  if (verbose) {
    ...
    cout << "Max alpha reached: " << result.maxAlphaReached << " (target: 1.0)"
         << endl;
    ...
    cout << "Gmin   = " << scientific << result.finalGmin << " S (want <=1e-12)"
         << endl;
  }
