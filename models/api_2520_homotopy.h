...
//   - sourceScale: 0→1 scales ALL independent sources (VCC, VEE, I_Tail, I_VAS,
//   I_Bias)
//   - alpha: 0→1 controls feedback strength (R_fb = R_nom / max(alpha,
//   ALPHA_MIN))
//   - bleederG: Gmin-like conductance from nC2→VEE to prevent latch
//
// WHY each change helps:
// 1. sourceScale from 0: At s=0 all sources are off, circuit is trivially at
// 0V.
//    As s increases, transistors gradually turn on from a balanced state.
//    Current sources (I_Tail, I_VAS) scaling is CRITICAL - if they're full
//    while supplies are ramping, the transistors try to sink current that
//    doesn't exist.
//
// 2. bleeder at nC2: When Q2 (diff pair NPN) is weakly conducting (low source
// scale),
...
public:
  ...
  struct Sources {
    ...
    Resistor *R_Out_Bleeder; // OUT→GND anti-latch bleeder
    Resistor *R_VAS_Bleeder; // VAS→GND anti-latch bleeder
  };
  ...
  static void build(Circuit &c, Sources &src, Nodes &nodes) {
    ...
    src.R_Feedback =
        &c.addElement<Resistor>("R_Feedback", nodes.OUT, nodes.INM,
    ...
    // === BLEEDER: Anti-latch paths ===
    // These prevent floating nodes during source ramp, faded to 0 at end
    src.R_Bleeder =
        &c.addElement<Resistor>("R_Bleeder_C2", nodes.C2, nodes.VEE, 1e6);
    src.R_Out_Bleeder =
        &c.addElement<Resistor>("R_Bleeder_OUT", nodes.OUT, GND, 1e6);
    src.R_VAS_Bleeder =
        &c.addElement<Resistor>("R_Bleeder_VAS", nodes.VAS, GND, 1e7);
    ...
    auto q2_nodes =
        addBjtExtended(c, nodes.C2, nodes.INM, nE2, q_npn, false, "Q2");
    ...
    src.I_Tail =
        &c.addElement<CurrentSource>("I_Tail", nodes.TAIL, nodes.VEE, 0.0);

    // === VAS ===
    auto q4_nodes =
        addBjtExtended(c, nodes.C4, nB4, nodes.E4, q_pnp, true, "Q4");
    ...
    src.I_VAS =
        &c.addElement<CurrentSource>("I_VAS", nodes.VCC, nodes.VAS, 0.0);
    ...
    src.I_BiasChain = &c.addElement<CurrentSource>("I_BiasChain_Sink",
                                                   nodes.B_LO, nodes.VEE, 0.0);
  ...
  static void updateSources(Sources &src, double sourceScale, double alpha,
                            double bleederG) {
    // Scale supplies
    ...
    // Set bleeder resistances (high resistance = low conductance = off)
    ...
    src.R_Out_Bleeder->setResistance(r_bleeder);
    src.R_VAS_Bleeder->setResistance(r_bleeder * 10.0); // VAS bleeder is weaker
  }
  ...
  struct Diagnostics {
    ...
    void print() const {
      ...
      cout << "  V(OUT)=" << V_OUT << " V(INM)=" << V_INM << " V(C2)=" << V_C2
           << " VBE_Q2=" << VBE_Q2;
  ...
  static Diagnostics getDiagnostics(const vector<double> &x, const Nodes &nodes,
                                    double finalGmin) {
    Diagnostics d;
    d.V_OUT = x[nodes.OUT];
    d.V_INM = x[nodes.INM];
  ...
  static bool isAcceptable(const Diagnostics &d) {
    return !d.isLatched && (d.finalGmin <= 1e-12) &&
           (d.VBE_Q2 > 0.45 && d.VBE_Q2 < 0.85) && (abs(d.V_OUT) < 2.0);
  }
};
