...
public:
  ...
  virtual void
  getDcConnections(vector<pair<int, int>> &connections) const = 0;
};

// Context for limiting operations
struct LimitContext {
  const vector<double> &x;    // Current Newton guess
  const vector<double> &xOld; // Previous converged solution (or initial)
};
...
public:
  ...
  virtual void stampNewton(StampContext &ctx,
                           const vector<double> &xGuess) const = 0;
  ...
  virtual void limitUpdate(const vector<double> &xNew) = 0;
};
...
public:
  ...
  virtual void commitStep(const vector<double> &xSolved) = 0;
};
