...
bool checkTransistorConditions(const std::vector<double>& x, int numNodes) {
    ...
    for (size_t i = 0; i < x.size(); ++i) {
        ...
        if (!std::isfinite(v)) {
            std::cout << "ERROR: Non-finite voltage at node " << i << ": " << v << std::endl;
            all_ok = false;
            continue;
        }
        ...
        if (std::abs(v) > 20.0) {
            std::cout << "WARNING: Large voltage at node " << i << ": " << v << "V (beyond ±20V)" << std::endl;
            all_ok = false;
        }

        // Check for negative voltages that are too negative (beyond VEE)
        if (v < -18.0) {
            std::cout << "WARNING: Very negative voltage at node " << i << ": " << v << "V" << std::endl;
            all_ok = false;
        }
    }
...
bool test2520DcOperatingPoint() {
    ...
    std::vector<double> x;
    Circuit::ConvergenceStats stats;

    if (!c.solveDc(x, 200, 1e-9, true, &stats)) {
        std::cout << "DC solve failed after " << stats.totalIterations << " iterations" << std::endl;
        return false;
    }

    std::cout << "DC convergence achieved in " << stats.totalIterations << " total iterations" << std::endl;
    ...
    if (std::abs(vout) > 2.0) {
        std::cout << "Output offset too large: " << vout << "V (should be near 0V)" << std::endl;
        return false;
    }

    std::cout << "Output voltage: " << std::fixed << std::setprecision(3) << vout << "V ✓" << std::endl;

    // Check that supplies are at correct voltages (±15V)
    // Note: We can't easily check internal node voltages without named node mapping
...
bool test2520InputOffset() {
    std::cout << "Testing 2520 Input Offset and Basic Functionality..." << std::endl;
    ...
    if (!c.solveDc(x, 100, 1e-9, true)) {
        std::cout << "Balanced input solve failed" << std::endl;
        return false;
    }

    double vout_balanced = x[out_node];
    std::cout << "Balanced output: " << std::fixed << std::setprecision(6) << vout_balanced << "V" << std::endl;

    // Check that output is reasonably close to 0V
    if (std::abs(vout_balanced) > 1.0) {
        std::cout << "Output offset too large with balanced inputs: " << vout_balanced << "V" << std::endl;
        return false;
    }
    ...
    std::cout << "Differential output (1mV in): " << std::fixed << std::setprecision(6) << vout_diff << "V" << std::endl;

    // Check that the circuit responds to input (basic functionality)
    if (std::abs(vout_diff - vout_balanced) < 0.001) { // At least 1mV change for 2mV input
    ...
    if (!checkTransistorConditions(x, c.getNumNodes())) {
        std::cout << "Abnormal operating conditions with differential input" << std::endl;
        return false;
    }
...
int main() {
    ...
    if (!test2520DcOperatingPoint()) all_passed = false;
    if (!test2520InputOffset()) all_passed = false;

    if (all_passed) {
        std::cout << "\n🎉 All 2520 tests PASSED! Ready for schematic refinement." << std::endl;
        return 0;
    } else {
        std::cout << "\n❌ Some 2520 tests FAILED. Need schematic corrections." << std::endl;
        return 1;
    }
}
